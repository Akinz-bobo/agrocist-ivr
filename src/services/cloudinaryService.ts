...
class CloudinaryService {
  ...
  private fileExistenceCache: Map<
    string,
  ...
  async uploadAudio(
    ...
    try {
      ...
      this.fileExistenceCache.set(result.public_id, {
        exists: true,
        timestamp: Date.now(),
      });
  ...
  async fileExists(publicId: string): Promise<boolean> {
    ...
    try {
      ...
      if (error.http_code === 404) {
        ...
        this.fileExistenceCache.set(publicId, {
          exists: false,
          timestamp: now,
        });
  ...
  getOptimizedUrl(
    ...
    try {
      ...
      const url = cloudinary.url(publicId, {
        ...
        transformation:
          transformations.length > 0 ? transformations.join(",") : undefined,
      });
  ...
  getCacheStats(): {
    ...
    const now = Date.now();
    const entries = Array.from(this.fileExistenceCache.entries()).map(
      ([publicId, data]) => ({
        publicId,
        exists: data.exists,
